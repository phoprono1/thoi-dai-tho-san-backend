// simulate-combat-large.ts
// Run a large number of combats and write a markdown report.

// Silence console logs from engine during bulk runs
const originalConsoleLog = console.log;
console.log = () => {};

// Use require to control order
const { runCombat } = require('../src/combat-engine/engine');
const { deriveCombatStats } = require('../src/combat-engine/stat-converter');
const fs = require('fs');
const path = require('path');

// Restore console for summary prints later
...
async function runLarge() {
  ...
  const avgLogs = totalLogs / results.length;
  ...
  report.push('- If average player skill damage is too high, review skill damageFormula for heavy skills (Quick Attack, Hỏa cầu).');
  report.push('- If missRate still high for some builds, consider lowering MAX_DELTA or adjusting accuracy softcap.');

  // Write report file
  const outPath = path.join(__dirname, `simulation-report-${new Date().toISOString().slice(0,10)}.md`);
  fs.writeFileSync(outPath, report.join('\n'));

  // Restore console and print summary
  restoreConsole();
  console.log('Simulation completed. Report written to', outPath);
  console.log('Summary:');
...
runLarge().catch((e) => {
  restoreConsole();
  console.error('simulate-combat-large failed', e);
  process.exit(1);
});
  export {};
